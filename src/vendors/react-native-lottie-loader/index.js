...
export default class Spinner extends React.PureComponent {
  ...
  renderLoader = () => {
    const {animationStyle, autoPlay, autoSize, loop, source, speed} =
      this.props;
  ...
  renderText = () => {
    const {textStyle, message} = this.props;

    return <Text style={[styles.textStyle, textStyle]}>{message}</Text>;
  };

  render() {
    const {animationType, style, visible, supportedOrientations} = this.props;
